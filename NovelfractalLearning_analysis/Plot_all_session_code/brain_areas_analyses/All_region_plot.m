...
%directories.variableDir = '.\segments';
...
% load(fullfile(directories.variableDir,'segmentmap.mat'));% load segmentNames
...
% sort the area by percentage of selective neuron/either positive or
% negative
...
if contains(currenttype, 'Nov_excited')
    included_Ind = find([datastruct.P_pred_nov_vs_fam] <= StatisticalThreshold & [datastruct.pred_nov_vs_fam] >0 );
elseif contains(currenttype, 'Nov_inhibited')
    included_Ind = find([datastruct.P_pred_nov_vs_fam] <= StatisticalThreshold & [datastruct.pred_nov_vs_fam] <0 );
elseif contains(currenttype, 'Novel_selective')
    included_Ind_pos = find([datastruct.P_pred_nov_vs_fam] <= StatisticalThreshold & [datastruct.pred_nov_vs_fam] >0);
    included_Ind_neg = find([datastruct.P_pred_nov_vs_fam] <= StatisticalThreshold & [datastruct.pred_nov_vs_fam] <0 );
    included_Ind = [included_Ind_pos,included_Ind_neg];
elseif contains(currenttype, 'All_included')
    included_Ind = 1:numel(datastruct);%find([datastruct.P_pred_nov_vs_fam] <= inf );
else
...
if contains(currenttype, 'Novel_selective')
    for ii = 1: numel(Indexname)
        Indices_struct.(Indexname{ii}) = [datastruct(included_Ind_pos).(pars.experimentindices{ii}),1-[datastruct(included_Ind_neg).(pars.experimentindices{ii})]];
        % index range [0, 1] here
    end
else
    for ii = 1: numel(Indexname)
        Indices_struct.(Indexname{ii}) = [datastruct(included_Ind).(pars.experimentindices{ii})];
    end
end
...
name2save_pdf = fullfile(plotpath,['Main_neuron_distribution_and_correlation_across_area_plot.pdf']);

...
plotplacesetx = {11:45,11:45,11:45,51:85,91:125,131:165,171:205,91:125,131:165, 171:205};
plotplacesety = {121:169, 1:49, 61:109, 61:109, 61:109, 61:109, 61:109,1:49,121:169, 1:49};
...
for i = 1:length(variablenames)
    eval(['All_FR_start =' variablenames{i} '_start;']);
    eval(['All_FR_end =' variablenames{i} '_end;']);
    nsubplot(300,210, plotplacesety{i}, plotplacesetx{i});hold on;
    plot([0.5,2.5], [0,0], 'color', [0.8,0.8,0.8])
    
    Datamean = [nanmean(All_FR_start), nanmean(All_FR_end)];
    Datastd = [nanstd(All_FR_start)/sqrt(length(All_FR_start)), nanstd(All_FR_end)/sqrt(length(All_FR_end))];
    ...
    if ~contains(variablenames{i},'Familiar')
        ...
        nsubplot(300,210, plotplacesety{i}+180, plotplacesetx{i});
        ...
        if contains(variablenames{i},'Learning_Novel')
            ...
            Learning_Novel_Datastd = Datastd;
        elseif contains(variablenames{i},'Learning') && contains(variablenames{i},'day')
            errorbar([1,2],Learning_Novel_Datamean, Learning_Novel_Datastd, 'r');
        end
